        ...
        <>
            <div className="grid grid-rows-1 gap-1 justify-center " style={{gridTemplateColumns: `repeat(${totalWeeks}, clamp(10px, 4vw, 16px))`}}>
                {visibleLabels.map((ml) => 
                    <div key={ml.column} className="w-4 text-white/70 text-sm min-w-0 overflow-hidden whitespace-nowrap" style={{gridColumnStart: ml.column + 1}}>{monthNames[ml.month]}</div>
                )}
            </div>
